...
- `[Question]` in the issue title — a proposed multiple-choice question, correct
  answer, optional distractors, and source document/page.
- `[Suggestion]` in the issue title — any interface, feature, map, picture, or
  other improvement.

## When the owner requests a suggestion update

1. Review open issues whose titles begin with `[Question]`.
...
Issues whose titles begin with `[Suggestion]` are never implemented
automatically. Summarize them for the owner and wait for explicit approval before
making a change.
